...
class Tugas_item extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
		$this->load->database();
        $this->load->model(array('Tugas_item_mod','Identitas_web_model'));
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'tugas_item/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'tugas_item/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'tugas_item/index.html';
            $config['first_url'] = base_url() . 'tugas_item/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Tugas_item_mod->total_rows($q);
        $tugas_item = $this->Tugas_item_mod->get_limit_data($config['per_page'], $start, $q);

        $this->load->library('pagination');
        $this->pagination->initialize($config);
            
        $this->data['tugas_item_data'] = $tugas_item;
        $this->data['q'] = $q;
		...
		$this->data['title'] = 'tugas_item';
		$this->get_Meta();
			
        $this->data['_view'] = 'tugas_item/tb_tugas_lists_list';
    ...
    public function read($id) 
    {
		...
		else
		{
			...
			$row = $this->Tugas_item_mod->get_by_id($id);
			if ($row) {
				...
				$this->data['title'] = 'tugas_item';
				$this->get_Meta();
				$this->data['_view'] = 'tugas_item/tb_tugas_lists_read';
				$this->_render_page('layouts/main',$this->data);
			} else {
				$this->data['message'] = 'Data tidak ditemukan';
				redirect(site_url('tugas_item'));
			}
		}
    }
    ...
    public function create() 
    {
		...
		else
		{
			...
			$this->data['action'] = site_url('tugas_item/create_action');
		    $this->data['id'] = array(
				'name'			=> 'id',
				'type'			=> 'text',
		    ...
		    $this->data['member'] = array(
				'name'			=> 'member',
				'type'			=> 'text',
				'value'			=> $this->form_validation->set_value('member'),
				'class'			=> 'form-control',
			);
			...
			$this->data['title'] = 'tugas_item';
			$this->get_Meta();
			$this->data['_view'] = 'tugas_item/tb_tugas_lists_form';
			$this->_render_page('layouts/main',$this->data);
		}
    }
    ...
    public function create_action() 
    {
		...
		'data' 			=> $this->input->post('data',TRUE),
            ...
            $this->Tugas_item_mod->insert($data);
    ...
    public function update($id) 
    {
		...
		else
		{
			...
			if ($row) {
				...
				$this->data['action']		= site_url('tugas_item/update_action');
			    $this->data['id'] = array(
					'name'			=> 'id',
					'type'			=> 'text',
    ...
    public function update_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->Tugas_item_mod->update($this->input->post('id', TRUE), $data);
	...
	foreach ($this->Tugas_item_mod->get_all() as $data) {
            $kolombody = 0;
...
/* End of file Tugas_item.php */
/* Location: ./application/controllers/Tugas_item.php */
